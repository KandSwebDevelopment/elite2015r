...
if(!$res->EOF){
  ...
  $res = $db->Execute("SELECT
        ...
        WHERE
          ...
          products.products_model <> '" . $model_code . "' AND
...
if ($num_images) {
  ...
  for ($i=0, $n=$num_images; $i<$n; $i++) {
    ...
    $thumb_slashes = zen_image($base_image, ($images_array[$i]['name']), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT);
...
if(true){
   //$script_link = '<script language="javascript" type="text/javascript"><!--' . "\n" . 'document.write(\'' . ($flag_display_large ? '<a href="' . zen_lightbox($products_image_large, addslashes($images_array[$i]['name']), LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT) . '" rel="lightbox[gallery]" title="' . addslashes($images_array[$i]['name']) . '">' . $thumb_slashes .  '</a>' : $thumb_slashes) . '\');' . "\n" . '//--></script>';
      //$script_link =  ($flag_display_large ? '<a href="' . zen_lightbox($products_image_large, addslashes($images_array[$i]['name']), LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT) . '" rel="lightbox[gallery]" title="' . addslashes($images_array[$i]['name']) . '">' . $thumb_slashes .  '</a>' : $thumb_slashes);

    $script_link = '<a href="' . zen_href_link(zen_get_info_page(zen_get_info_page($images_array[$i]['id'])), ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $images_array[$i]['id']) . '">' . $thumb_slashes . '</a>' ;

    //work out this items position in the family
    $fp=1+$i;
    if($i>=$current_family_pos-1)$fp=2+$i;

    $script_link .= '<br/><a href="'.  zen_href_link(zen_get_info_page($images_array[$i]['id']), ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $images_array[$i]['id']) .'"> <div class="familyViewItem"><span class="familyItemsCount" />Item '. $fp . ' of ' . $max_family_count . '</span><br />' . $images_array[$i]['name']. '</div></a>';
