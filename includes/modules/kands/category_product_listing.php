<?php
/**
* KandS - As this is to display a list of all products that are in this 
* categories, sub-categories, it has to produce the correct sql script 
* ($listing_sql) to pull the products. It is based on the code from 
* index\main_template_vars.php and index_filters\default_filter.php
*/
...
if ($listing_split->number_of_rows > 0) {
	$rows = 0;
	if($_GET['kands']==1)echo $listing_split->sql_query;
	...
	while (!$listing->EOF) {
		...
		for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
			...
			switch ($column_list[$col]) {
				...
				case 'PRODUCT_LIST_NAME':
				$lc_align = '';
				$lc_text = '<div class="product_title"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' .  zen_get_generated_category_path_rev($listing->fields['master_categories_id']) . '&products_id=' . $listing->fields['products_id']) . '">' . $listing->fields['products_name'] . '</a></h3><div class="listingDescription">' . zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION) . '</div>';
				break;
				case 'PRODUCT_LIST_MANUFACTURER':
				$lc_align = '';
