...
if ($num_categories > 0) {
  if ($num_categories < MAX_DISPLAY_CATEGORIES_PER_ROW || MAX_DISPLAY_CATEGORIES_PER_ROW == 0) {
    $col_width = floor(100/$num_categories)-2;//KAS - Added -2 to decrease the width for the cols for the sub cats 
  } else {
    $col_width = floor(100/MAX_DISPLAY_CATEGORIES_PER_ROW) -2;//KAS - Added -2 to decrease the width for the cols for the sub cats 
  }
