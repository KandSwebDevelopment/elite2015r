...
function check_terms(){
	if (form.elements['privacy'] && (form.elements['privacy'].type != "hidden")) {
	    //var field_value = form.elements['privacy'].value;

	    //if (field_value == 0) {
	    if(!form.elements['privacy'].checked){
	      error_message = error_message + "* " + 'Please agree to our terms and conditions' + "\n";
	      error = true;
	      changeColor('privacy');
	    }
	  }
...
function check_trade_type(){
    ...
    var sVal = form.elements["trade_type"].value;
    if(sVal==0){
...
function check_form(form_name) {
  ...
  validateEmail('create_account','email_address');
...
<?php } ?>
  ...
  if (error == true) {
    ...
    return false;
  	...
  	$j("#fuzz").css("height", $j(document).height());
  	$(".tac_popup").center();
  	$j("#fuzz").fadeIn();
    //submitted = true;
    return false;
