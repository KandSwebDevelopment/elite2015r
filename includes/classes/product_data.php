...
class ProductData {
  ...
  var  $specs_map= array(
    ...
    'bulbs_qty' => array('name' =>'Bulbs/Lamps Quantity',  'postfix' => ''),
    'bulbs_watts' => array('name' =>'Bulb/Lamp Wattage',  'postfix' => 'w'),
    'bulbs_type' => array('name' =>'Bulb/Lamp Type',  'postfix' => ''),
    'bulbs_cap' => array('name' =>'Bulbs/Lamps End Cap',  'postfix' => ''),
    ...
    'product_surface_temp' => array('name' =>'Surface Temperature',  'postfix' => '&deg;C'),
    ...
    'lumens' => array('name' =>'Lumens',  'postfix' => 'lm'),
    'colour_temp' => array('name' =>'Colour Temperature',  'postfix' => 'K'),
    'energy_class' => array('name' =>'Energy Class',  'postfix' => ''),
    'cri' => array('name' =>'Colour Rendering Index',  'prefix' => '>'),
    'hours' => array('name' =>'Life Span',  'postfix' => 'Hours')
    );
      ...
      if(isset($this->specs_map[$key])){
        if($this->specs_map[$key]['type'] == 'logic'){
          $res[$this->specs_map[$key]['name']] = $value==1?'Yes':'No';
					...
					if(	((is_string($value)&& ($value != '') && ($value != 'NULL') && (strlen(trim($value)) > 0)&&($value!="0")))
						or
        		((is_int($value)) && ($value != '') && ($value != 'NULL') && (intval($value!=0)))
						)
        		{
        			//Get any prefix
		          ...
		          //Get any postfix
		          if(zen_not_null($this->specs_map[$key]['postfix'])){
...
/**
* Made redundant 16/6/14 as all the bulb cols ie cap, wattage, qty and type are no
* longer used. The bulb_s1 string now holds this info ready for output
*
* @param mixed $pID
*/
function get_bulb_string($pID){
    global $db;
    $sql = "SELECT bulbs_s1 FROM " . TABLE_PRODUCTS_EXTRA_FIELDS . " WHERE products_id  = $pID";
    $result = $db->Execute($sql);
    if(!$result->EOF){
      $string = '<br /><br />';
