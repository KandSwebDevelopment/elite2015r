...
',
		...
		function buildBranch($parent_id, $level, $submenu=true, $parent_link='', $class='', $sub_class = '')
		{
			if($class==''){
				$result = sprintf($this->parent_group_start_string, ($submenu==true) ? ' class="level'. ($level+1) . '"' : '' );
			}else{
				$result = sprintf($this->parent_group_start_string, ($submenu==true) ? ' class="'. ($class) . '"' : '' );        
			}
						...
						foreach($this->data[$parent_id] as $category_id => $category) {
								...
								if (($this->data[$category_id])) {
									if($sub_class==''){
										...
										$result .= sprintf($this->child_start_string, ($submenu==true) ? ' class="'.$sub_class.'"' : '');
									}
								} else {
		...
		function buildTree2($submenu=false)
		{
				return $this->buildBranch($this->root_category_id, '', $submenu, "dl-menu","dl-submenu");
		}
}
?>